  ...
  it("writes stable JSON, Markdown, and CSV report outputs in mock mode", async () => {
    ...
    expect(
      report.results.every(
        (result) => result.caseId && result.runId && typeof result.qualityScore === "number" && typeof result.durationMs === "number" && result.failureCategory,
      ),
    ).toBe(true);
...
describe("generated app regression coverage", () => {
  it("renders API routes, seed data, fields, statuses, and run commands", () => {
    ...
    expect(server).toContain('send(response, 500, { error: "Unexpected server error." });');
